...
import { IndianRupee, Banknote, Smartphone, CircleCheck, Settings2, KeyRound, X, TrendingUp, ListPlus, Pencil, Trash2, Plus } from "lucide-react";
...
export default function Owner() {
  ...
  const chartData = [
    { name: "Cash", value: stats.cash_amount, fill: "hsl(var(--accent))" },
    { name: "Online", value: stats.online_amount, fill: "hsl(var(--primary))" },
  ];
      ...
      <div className="mt-8">
        <div className="flex items-end justify-between mb-3">
          <div>
            ...
            <div className="font-display font-bold text-xl">Today's log</div>
          </div>
          <div className="text-xs text-muted-foreground">{completed.length} entries</div>
        </div>
        <div className="grid gap-3">
          {completed.length === 0 && (
            <div className="bg-card border border-dashed border-border rounded-[18px] p-10 text-center text-muted-foreground">No completions yet today.</div>
          )}
          {completed.slice(0, 20).map((b, i) => (
            <motion.div
              key={b.id}
            ...
            >
              ...
              <div className="flex-1 min-w-[180px]">
                ...
                <div className="font-mono text-xs text-muted-foreground mt-0.5">{b.vehicle_number} · {b.service_name} · ₹{b.price}</div>
              ...
              <div className="flex items-center gap-2">
                <span className={`chip ${b.payment_method === "cash" ? "bg-accent/15 border-accent/30" : "bg-primary/10 text-primary border-primary/20"}`}>
                  {b.payment_method === "cash" ? "cash" : (b.payment_provider || "online")}
                </span>
                {b.worker_photo && (
        ...
        {!authed ? (
          <div className="p-8">
            ...
            <div className="flex gap-2">
              <input data-testid="svc-mgr-pin" inputMode="numeric" value={ownerPin} onChange={(e) => setOwnerPin(e.target.value.replace(/\D/g, "").slice(0, 6))} className="input-field font-mono" placeholder="••••" />
